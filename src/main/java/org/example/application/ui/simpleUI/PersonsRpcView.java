...
@Getter
@Setter
public class PersonsRpcView implements RpcCrudView<PersonsRpcView, PersonsRpcView.Row, Person> {
    ...
    @Override
    public List<Row> rpc(PersonsRpcView filters, List<QuerySortOrder> sortOrders, int offset, int limit) throws Throwable {
        return JPAHelper.findAll(Person.class).stream().map(p -> new Row("" + p.getId(), p.getName(), "xxx")).collect(Collectors.toList());
    }

    @Override
    public int gatherCount(PersonsRpcView filters) throws Throwable {
